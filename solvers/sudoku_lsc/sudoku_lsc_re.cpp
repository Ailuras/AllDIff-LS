...
void set_Variable_size() {
    ...
    mVertexesMark = (int *) malloc(sizeof(int) * (vertex_size + 2));
    mVertexesMarkTmp = (int *) malloc(sizeof(int) * (vertex_size + 2));
...
void add_vertex(int id, int pos) {
	...
	for(int i = 0; i < Neighbour_length[id]; i++) {
        ...
        //         if (vertex_color[tmp_v][j] == pos) {
        //             mVertexesMark[tmp_v] = 0;
        //             break;
        //         }
        //     }
        // }else {
...
int tabuSearch() {
	...
	for(int i = 0; i < vertex_use_length; i++) {
        ...
        mVertexesMarkTmp[mVertexes[i]] = 0;
    }
	int min_sub_clash;
	...
	for (long long iters = mPopulationMaxVOld[mPopulation_id] + 1; iters < total_iters; iters++) {
		...
		// 当被禁忌的最小值比不被禁忌的最小值小、并且采用该move比当前最小值还小时，或者不存在不被禁忌的好move时，进入该循环；
			...
			// for (int i=0; i<cnt; i++) {
			// 	cout << "v: " << tmp_tabu_st[i].id << ", c: " << tmp_tabu_st[i].c << ", score: " << tmp_tabu_st[i].score << endl;
			// }
			int r;
		...
		// 否则就选择非禁忌的操作
			...
			// for (int i=0; i<cnt; i++) {
			// 	cout << "v: " << tmp_st[i].id << ", c: " << tmp_st[i].c << ", score: " << tmp_st[i].score << endl;
			// }
			int r;
		...
		if (total_clash <= tSminClash) {
			...
			for(int i = 0; i < vertex_use_length; i++) {
				mVertexesColorTmp[mVertexes[i]] = mVertexesColor[mVertexes[i]];
                mVertexesMarkTmp[mVertexes[i]] = mVertexesMark[mVertexes[i]];
			}
        ...
        // if (no_improve_iters > max_no_iter) return tSminClash;
	}
	return tSminClash;
}
...
void mPopulationUpdate() {
	...
	if(clash_cur > best_clash) return;
...
void mPerturbation() {
	...
	// 	int x = edge[best_clash_array[i].e_id].x;
	// 	int y = edge[best_clash_array[i].e_id].y;
    //     int c = best_clash_array[i].color;
    //     for (int j=0; j<Neighbour_length[x]; j++) {
    //         int tmp_v = Neighbour[x][j];
    //         for (int k=0; k<vertex_color_length[tmp_v]; k++) {
    //             if (vertex_color[tmp_v][k] == c) {
    //                 mVertexesColor[tmp_v] = vertex_color[tmp_v][rand()%vertex_color_length[tmp_v]];
    //                 cnt++;
    //             }
    //         }
    //     }
    //     for (int j=0; j<Neighbour_length[y]; j++) {
    //         int tmp_v = Neighbour[y][j];
    //         for (int k=0; k<vertex_color_length[tmp_v]; k++) {
    //             if (vertex_color[tmp_v][k] == c) {
    //                 mVertexesColor[tmp_v] = vertex_color[tmp_v][rand()%vertex_color_length[tmp_v]];
    //                 cnt++;

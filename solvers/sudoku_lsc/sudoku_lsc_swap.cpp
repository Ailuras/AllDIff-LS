...
void set_Variable_size() {
	...
	mSquColor = (int **) malloc(sizeof(int *) * (square_size + 1));
	for(int i = 1; i <= square_size; i++) mSquColor[i] = (int *) malloc(sizeof(int) * (square_size + 1));
	visited = (bool *) malloc(sizeof(bool) * (square_size + 1));
	...
	mVertexes_pos = (int *) malloc(sizeof(int) * (vertex_size + 2));
	...
	mEdges_pos = (int *) malloc(sizeof(int) * (edge_size + 2));
	...
	tmp_bms_st = (St *) malloc(sizeof(St) * (square_size * vertex_size + 1));
...
void init() {
	...
	for(int i = 1; i <= square_size; i++) for(int j = 1; j <= square_size; j++) mSquColor[i][j] = -1;
...
void swap_vertex(int id_a, int pos_b) {
	...
	int pos_a = mVertexes_pos[id_a];
	mVertexes[pos_a] = id_b;
	mVertexes_pos[id_a] = pos_b;
	mVertexes[pos_b] = id_a;
	mVertexes_pos[id_b] = pos_a;
	return;
}

void swap_edge(int id_a, int pos_b) {
	int id_b = mEdges[pos_b];
	int pos_a = mEdges_pos[id_a];
	mEdges[pos_a] = id_b;
	mEdges_pos[id_a] = pos_b;
	mEdges[pos_b] = id_a;
	mEdges_pos[id_b] = pos_a;
	return;
}
...
void add_vertex(int id, int pos) {
	...
	for(int i = 0; i < Neighbour_length[id]; i++) {
		...
		if(mVertexesColor[tmp_v] == cur_pos) {
			if(mClash[tmp_v][cur_pos] == 0) {
				mClashVertexes[ClashVertexesPos[tmp_v]] = mClashVertexes[--vertex_clash_length];
				ClashVertexesPos[mClashVertexes[ClashVertexesPos[tmp_v]]] = ClashVertexesPos[tmp_v];
				ClashVertexesPos[tmp_v] = -1;
				score[tmp_v] = 0;
			}
			swap_edge(edge_id, mClashEdges_length--);
		}
		else if(mVertexesColor[tmp_v] == pos) {
			if(ClashVertexesPos[tmp_v] == -1) {
				mClashVertexes[vertex_clash_length] = tmp_v;
	...
	mSquColor[get_Squ(id)][pos] = id;
	if(mClash[id][pos] == 0 && ClashVertexesPos[id] >= 0) {
		mClashVertexes[ClashVertexesPos[id]] = mClashVertexes[--vertex_clash_length];
		ClashVertexesPos[mClashVertexes[ClashVertexesPos[id]]] = ClashVertexesPos[id];
...
void check_vertex_clash() {
	...
	for(int i = 1; i <= edge_size; i++) {
		tmp_x = edge[i].x;
		tmp_y = edge[i].y;
		if(mVertexes_pos[tmp_x] == -1 || mVertexes_pos[tmp_y] == -1) continue;
...
void check_clash() {
	int res = 0;
	for(int i = 1; i <= edge_size; i++) {
		if(mVertexes_pos[edge[i].x] == -1 || mVertexes_pos[edge[i].y] == -1) continue;
			...
			for(int tmp_c = 1; tmp_c <= square_size; tmp_c++) {
				if(vertex_color_length[vertex_id] == 1 || mCol_length[get_Col(vertex_id)][tmp_c] == 1 || mCol_length[get_Col(vertex_id)][tmp_c] == 1 || mCol_length[get_Col(vertex_id)][tmp_c] == 1) {
					...
					while(!q.empty()) {
						...
						mSquColor[get_Squ(tmp_v)][tmp_c] = tmp_v;
	...
	for (int i=0; i<vertex_use_length; i++) {
		...
		for (int j=0; j<Neighbour_length[vertex_id]; j++) {
			...
			if (mVertexesColor[tmp_v] > 0) {
				...
				if(mEdges_pos[edge_id] > mEdges_length) swap_edge(edge_id, ++mEdges_length);
			}
		}
	}
	for (int i=1; i<=square_size; i++) {
...
void build() {
	...
	for(int i = 1; i <= mEdges_length; i++) {
		...
		if(mVertexesColor[edge[mEdges[i]].x] == mVertexesColor[edge[mEdges[i]].y]) {
			...
			total_clash += 1;
			swap_edge(mEdges[i], ++mClashEdges_length);
		}
	}
	for(int i = 0; i < vertex_use_length; i++) {
	...
	for(int i = 0; i < vertex_use_length; i++) {
		...
		if(mClash[vertex_id][mVertexesColor[vertex_id]] > 0) {
			mClashVertexes[vertex_clash_length] = vertex_id;
...
void increase_score() {
	for(int i = 1; i <= mClashEdges_length; i++) {
	...
	for (long long iters = mPopulationMaxVOld[mPopulation_id] + 1; iters < total_iters; iters++) {
		...
		for (int i = 0; i < vertex_clash_length; i++) {
			...
			for(int j = 0; j < vertex_color_length[vertex_id]; j++) {
				...
				int swap_id = mSquColor[get_Squ(vertex_id)][tmp_c];
				if (edge[G[vertex_id][swap_id]].x == swap_id) continue;
				bool flag = true;
				for (int k=0; k<vertex_color_length[vertex_id]; k++) {
					if (vertex_color[vertex_id][k] == tmp_c) {
						flag = false;
						break;
					}
				}
				if (flag) continue;
				// if (get_Row(vertex_id) > get_Row(swap_id) || (get_Row(vertex_id) == get_Row(swap_id) && get_Col(vertex_id) > get_Col(swap_id))) continue;
				// 计算得分
				int tmp_s = get_score(vertex_id, tmp_c) + get_score(swap_id, mVertexesColor[vertex_id]);
				//long long tmp_cs = 0;
				// 不在禁忌列表的话
		...
		// 当被禁忌的最小值比不被禁忌的最小值小、并且采用该move比当前最小值还小时，或者不存在不被禁忌的好move时，进入该循环；
			...
			for(int i = 0; i < s_cnt; i++) {
				...
				else if(min_sub_tabu_cs == tmp_cs) tmp_bms_st[cs_cnt++] = tmp_tabu_st[r];
			}
			r = rand() % cs_cnt;
			move_id1 = tmp_bms_st[r].id;
			move_to = tmp_bms_st[r].c;
			move_from = mVertexesColor[move_id1];
			move_id2 = mSquColor[get_Squ(move_id1)][move_to];
			// 更新tabu表，它由三部分组成
			tabu[move_id1][mVertexesColor[move_id1]] = iters + rand() % tabuStep + alpha * vertex_clash_length;
			tabu[move_id2][mVertexesColor[move_id2]] = iters + rand() % tabuStep + alpha * vertex_clash_length;
			// 实施move
			// add_vertex(move_id, move_to);
			add_vertex(move_id1, move_to);
			add_vertex(move_id2, move_from);
			// printf("swap %d(square:%d, color:%d) and %d(square:%d, color:%d)\n", move_id1, get_Squ(move_id1), move_from, move_id2, get_Squ(move_id2), move_to);
			total_clash += min_sub_tabu_clash;
			if(vertex_old[move_id1][1] <= update_Iter) vertex_old[move_id1][0] = vertex_old[move_id1][1];
			vertex_old[move_id1][1] = iters;
			if(vertex_old[move_id2][1] <= update_Iter) vertex_old[move_id2][0] = vertex_old[move_id2][1];
			vertex_old[move_id2][1] = iters;
			// updating rule 1
			increase_score();
			score[move_id1] = 0;
			score[move_id2] = 0;
		...
		// 否则就选择非禁忌的操作
			...
			for(int i = 0; i < s_cnt; i++) {
				...
				else if(min_sub_cs == tmp_cs) tmp_bms_st[cs_cnt++] = tmp_st[r];
			}
			r = rand() % cs_cnt;
			move_id1 = tmp_bms_st[r].id;
			move_to = tmp_bms_st[r].c;
		...
		if (total_clash <= tSminClash) {
			...
			for(int i = 1; i <= mClashEdges_length; i++) best_clash_array[i - 1].e_id = mEdges[i];
			for(int i = 1; i <= mClashEdges_length; i++) best_clash_array[i - 1].color = mVertexesColor[edge[mEdges[i]].x];
...
bool match(int vertex_id) {
	for (int i=0; i<vertex_color_length[vertex_id]; i++) {
		if (!visited[vertex_color[vertex_id][i]]) {
			visited[vertex_color[vertex_id][i]] = true;
			if (mSquColor[get_Squ(vertex_id)][vertex_color[vertex_id][i]] == -1 || match(mSquColor[get_Squ(vertex_id)][vertex_color[vertex_id][i]])) {
				mSquColor[get_Squ(vertex_id)][vertex_color[vertex_id][i]] = vertex_id;
				mVertexesColor[vertex_id] = vertex_color[vertex_id][i];
				return true;
			}
		}
	}
...
void mGenerate() {
	for (int i=0; i<vertex_use_length; i++) {
		for (int i=0; i<=square_size; i++) visited[i] = false;
		match(mVertexes[i]);
	}
	// print_current();
...
void mCopy() {
	...
	//复制点的赋值
	for(int i = 0; i < vertex_use_length; i++) {
		color = mPopulation[mPopulation_id][mVertexes[i]];
		mVertexesColor[mVertexes[i]] = color;
		mSquColor[get_Squ(mVertexes[i])][color] = mVertexes[i];
	}
	//复制best_clash_array
	for(int i = 0; i < best_clash; i++) {
...
void mPerturbation() {
	...
	for(int i = 0; i < p_cnt; i++) {
		int r = rand() % cnt;
		mSquColor[get_Squ(tmp_array[r])][mVertexesColor[tmp_array[r]]] = -1;
		mVertexesColor[tmp_array[r]] = -1;
		tmp_array[r] = tmp_array[--cnt];
	}
	cnt = 0;
	for (int i=0; i<vertex_use_length; i++) {
		if (mVertexesColor[mVertexes[i]] == -1) tmp_array[cnt++] = mVertexes[i];
	}
	for (int i=0; i<cnt; i++) {
		for (int i=0; i<=square_size; i++) visited[i] = false;
		match(tmp_array[i]);

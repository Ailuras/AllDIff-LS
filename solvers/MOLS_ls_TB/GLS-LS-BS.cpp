...
void mRead(string filename) {
	...
	for (int i=1; i<=order; i++) {
		int var1 = i;
		var_color_length[var1] = 1;
		var_color[var1][0] = i;
		var_color_pos[var1][i] = 0;
		int var2 = i+order*order;
		var_color_length[var2] = 1;
		var_color[var2][0] = i;
		var_color_pos[var2][i] = 0;
		int var3 = 1+(i-1)*order;
		var_color_length[var3] = 1;
		var_color[var3][0] = i;
		var_color_pos[var3][i] = 0;
	}
...
void mReduceVertexes() {
	for (int i=1; i<=var_size; i++) {
		if (var_color_length[i] == 1) {
			// Variable operation
			int c = var_color[i][0];
			// Simplification operation var -> exp
			for (int k=0; k<mNeighbours_length[i]; k++) {
				int tmp_v = mNeighbours[i][k];
				if (var_color[tmp_v][var_color_pos[tmp_v][c]] == c && var_color_pos[tmp_v][c] < var_color_length[tmp_v]) {
					// var_color[tmp_v][var_color_pos[tmp_v][c]] == c && 
					swap_var_color(tmp_v, c, --var_color_length[tmp_v]);
				}
			}
			// Simplification operation 2*var -> exp
			// int exp_id = var_exp[i][1];
			// for (int k=0; k<mNeighbours_length[exp_id]; k++) {
			// 	int tmp_v = mNeighbours[exp_id][k];
			// 	int tmp_x = exp_var[tmp_v][0];
			// 	if (var_color_pos[tmp_v][c] < var_color_length[tmp_v]) swap_var_color(tmp_v, c, --var_color_length[tmp_v]);
			// }
		}
	}
	...
	// 		if (var_exp_length[exp_var[x][j]] > 1) {
	// 			f = true; break;
	// 		}
	// 	}
	// 	for (int j=0; j<exp_var_length[y]; j++) {
	// 		if (var_exp_length[exp_var[y][j]] > 1) {
	// 			f = true; break;
	// 		}
	// 	}
	// 	if (f) mEdges[mEdges_length++] = i;
	// }
	return;
}
		...
		if (exp_var_length[exp] == 1) {
			...
			for (int j=0; j<mNeighbours_length[exp]; j++) {
				...
				if (mVarsColor[tmp_v] == cur_pos) {
					...
					if (mClashVars[mClashVars_pos[tmp_v]] != tmp_v && var_color_length[tmp_v] > 1) {
			...
			for (int j=0; j<mNeighbours_length[exp]; j++) {
				...
				if (mVarsColor[x2] == mVarsColor[y2]) {
					...
					} else if (mVarsColor[y1] == pos && mClashVars[mClashVars_pos[y1]] != y1 && var_color_length[y1] > 1) {
				...
				if (cur_pos == mVarsColor[y1]) {
					...
					mClash[y2][mVarsColor[x2]] += edges[edge_id].w;
					if (mVarsColor[x2] == mVarsColor[y2]) {
						if (mClashVars[mClashVars_pos[x2]] != x2 && var_color_length[x2] > 1) {
							mClashVars[mClashVars_length] = x2;
							mClashVars_pos[x2] = mClashVars_length++;
						}
						if (mClashVars[mClashVars_pos[y2]] != y2 && var_color_length[y2] > 1) {
...
void switch_mode(int mode) {
	if (mode == 0) {
		...
		for (int i = 0; i < mVars_length; i++) {
			int var_id = mVars[i];
			if (mClash[var_id][mVarsColor[var_id]] > 0 && var_color_length[var_id] > 1) {

...
#elif 0				/* less slow but not as fast as above (from far !) */
...
Init_Parameters(AdData *p_ad)
{
  ...
  if (p_ad->reset_limit == -1)
    p_ad->reset_limit = (order < 12) ? 4 : 10;	/* so no reset occurs generally except of little values */
#else
  if (p_ad->reset_limit == -1)
    p_ad->reset_limit = 3;
#endif
  ...
  if (p_ad->reset_percent == -1)
    p_ad->nb_var_to_reset = 1;			      /* no reset occur */
#else
  if (p_ad->reset_percent == -1)
    p_ad->nb_var_to_reset = (order % 2 == 0) ? 2 : 1; // 2 if even / 1 if odd
#endif

  if (p_ad->restart_limit == -1) /* pefer to stop when no chance (for little values) */
    p_ad->restart_limit = 100000;

  if (p_ad->restart_max == -1)	/* and restart some times */
    p_ad->restart_max = 100;
}

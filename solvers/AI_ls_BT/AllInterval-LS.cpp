...
void set_Variable_size() {
	...
	mTabuX = (int *) malloc(sizeof(int) * (exp_size + 1));
...
bool check_tabuX(int id, int iter) {
	if (mTabuX[id] <= iter) return true;
	return false;
}
...
int mTabuSearch() {
	...
	for (int iters = 1; iters <= mPoolMaxIter[last_id]; iters++) {
		...
		if (false) {
			...
			mTabu[move_id][move_from] = iters + rand() % tabuStep + alpha2 * mExcelVars_length;
			...
			mTabuX[move_id] = iters + rand() % tabuStep + alpha2 * mClashVars_length;

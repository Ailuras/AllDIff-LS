...
void set_Variable_size() {
	...
	nscore = (long long *) malloc(sizeof(long long) * (edge_size + 1));
...
void check_answer() {
	...
	for (int i=0; i<mVars_length; i++) {
		int v = mVars[i];
		int tmp_c = mVarsColor_tmp[mVars[i]];
...
void update_info(int id, int pos) {
	...
	for (int i = 0; i < mNeighbours_length[v1]; i++) {
		...
		mClash[tmp_v][to_pre_color(tmp_v, cur_c1, true)] -= edges[edge_id].w;
		mClash[tmp_v][to_pre_color(tmp_v, c1, true)] += edges[edge_id].w;
		if (mVertexesColor[tmp_v] == to_pre_color(tmp_v, cur_c1, true)) {
			if (mClash[tmp_v][to_pre_color(tmp_v, cur_c1, true)] == 0) {
				mClashVars[mClashVars_pos[tmp_v]] = mClashVars[--mClashVars_length];
				mClashVars_pos[mClashVars[mClashVars_length]] = mClashVars_pos[tmp_v];
				mClashVars_pos[tmp_v] = -1;
				nscore[tmp_v] = 0;
            ...
            mClashEdges_pos[mClashEdges[mClashEdges_length]] = mClashEdges_pos[edge_id];
            mClashEdges_pos[edge_id] = -1;
		} else if (mVertexesColor[tmp_v] == to_pre_color(tmp_v, c1, true)) {
			if (mClashVars_pos[tmp_v] == -1) {
				mClashVars[mClashVars_length] = tmp_v;
				mClashVars_pos[tmp_v] = mClashVars_length++;
			}
	...
	for (int i = 0; i < mNeighbours_length[v2]; i++) {
		...
		mClash[tmp_v][to_pre_color(tmp_v, cur_pos, false)] -= edges[edge_id].w;
		mClash[tmp_v][to_pre_color(tmp_v, pos, false)] += edges[edge_id].w;
		if (mVertexesColor[tmp_v] == to_pre_color(tmp_v, cur_pos, false)) {
			if (mClash[tmp_v][to_pre_color(tmp_v, cur_pos, false)] == 0) {
				mClashVars[mClashVars_pos[tmp_v]] = mClashVars[--mClashVars_length];
				mClashVars_pos[mClashVars[mClashVars_length]] = mClashVars_pos[tmp_v];
				mClashVars_pos[tmp_v] = -1;
				nscore[tmp_v] = 0;
            ...
            mClashEdges_pos[mClashEdges[mClashEdges_length]] = mClashEdges_pos[edge_id];
            mClashEdges_pos[edge_id] = -1;
		} else if (mVertexesColor[tmp_v] == to_pre_color(tmp_v, pos, false)) {
			if (mClashVars_pos[tmp_v] == -1) {
				mClashVars[mClashVars_length] = tmp_v;
				mClashVars_pos[tmp_v] = mClashVars_length++;
			}
...
void build() {
	...
	for(int i = 0; i < mEdges_length; i++) {
		...
		if (type == 0) {
			...
			mClash[x][to_pre_color(x, mVertexesColor[y], true)] += edges[e].w;
			mClash[y][to_pre_color(y, mVertexesColor[x], true)] += edges[e].w;
		} else {
			x = to_index(edges[e].x);
			y = to_index(edges[e].y);
			mClash[x][to_pre_color(x, mVertexesColor[y], false)] += edges[e].w;
			mClash[y][to_pre_color(y, mVertexesColor[x], false)] += edges[e].w;
		}
...
void update_nscore() {
	for (int i=0; i<mClashEdges_length; i++) {
		nscore[edges[mClashEdges[i]].x] += 1;
		nscore[edges[mClashEdges[i]].y] += 1;
		// nscore[edges[mClashEdges[i]].x] += edges[mClashEdges[i]].w;
		// nscore[edges[mClashEdges[i]].y] += edges[mClashEdges[i]].w;
	}
	return;
}
		...
		if (true) {
			...
			if (cnt == 0 && tabu_cnt == 0) {
				...
				for(int i = 0; i < v_cnt; i++) {
					r = rand() % tabu_cnt;
					long long tmp_cs = nscore[tmp_tabu_st[r]];
					if (max_cs < tmp_cs) {
						cs_cnt = 0;
						max_cs = tmp_cs;
				...
				for(int i = 0; i < v_cnt; i++) {
					r = rand() % cnt;
					long long tmp_cs = nscore[tmp_st[r]];
					if (max_cs < tmp_cs) {
						cs_cnt = 0;
						max_cs = tmp_cs;

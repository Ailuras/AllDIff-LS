...
void set_Variable_size() {
	...
	alldiff = (int **) malloc(sizeof(int *) * (3));
	for(int i = 0; i <= 3; i++) alldiff[i] = (int *) malloc(sizeof(int) * (square_size + 1));
...
void mRead(string filename, string proportion) {
	...
	FILE *fp2 = freopen(proportion.c_str(), "r", stdin);
	for (int i=0; i<3; i++) {
...
void mReduceVertexes() {
	for (int i=1; i<=vertex_size; i++) {
		if (vertex_color_length[i] == 1) {
			...
			if (alldiff[0][get_Col(i)-1] == 1) mCol_length[get_Col(i)][tmp_c] = 0;
			swap_row(get_Row(i), tmp_c, i, 1);
			if (alldiff[1][get_Row(i)-1] == 1) mRow_length[get_Row(i)][tmp_c] = 0;
			swap_squ(get_Squ(i), tmp_c, i, 1);
			if (alldiff[2][get_Squ(i)-1] == 1) mSqu_length[get_Squ(i)][tmp_c] = 0;
	...
	for (int i = 1; i <= square_size; i++) {
		for (int j = 1; j <= square_size; j++) {
			if (mCol_length[i][j] == 1 && alldiff[0][i] == 1) {
				int tmp_v = mCol[i][j][1];
				swap_vertex_color(tmp_v, j, 0);
				q.push(tmp_v);
			}	
			if (mRow_length[i][j] == 1 && alldiff[1][i] == 1) {
				int tmp_v = mRow[i][j][1];
				swap_vertex_color(tmp_v, j, 0);
				q.push(tmp_v);
			}
			if (mSqu_length[i][j] == 1 && alldiff[2][i] == 1) {
	...
	while (!q.empty()) {
		...
		// todo2
		if (alldiff[0][get_Col(tmp_v)-1] == 1) mCol_length[get_Col(tmp_v)][tmp_c] = 0;
		if (alldiff[1][get_Col(tmp_v)-1] == 1) mRow_length[get_Row(tmp_v)][tmp_c] = 0;
		if (alldiff[2][get_Col(tmp_v)-1] == 1) mSqu_length[get_Squ(tmp_v)][tmp_c] = 0;
		for(int i = 1; i <= square_size; i++) {
	...
	// cout << "mVertexes_length = " << mVertexes_length << endl;
	// cout << "mEdges_length = " << mEdges_length << endl;
	return;
}

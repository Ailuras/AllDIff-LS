...
void set_Variable_size() {
	...
	bms_st = (St *) malloc(sizeof(St) * (square_size * vertex_size + 1));
...
void add_vertex(int id, int pos) {
	...
	for(int i = 0; i < Neighbour_length[id]; i++) {
		...
		if(mVertexesColor[tmp_v] == cur_pos) {
			...
			swap_edge(edge_id, mEdgeClash_length--);
		}
...
void build() {
	...
	for(int i = 1; i <= mEdges_length; i++) {
		...
		if(mVertexesColor[edge[mEdges[i]].x] == mVertexesColor[edge[mEdges[i]].y]) {
			...
			swap_edge(mEdges[i], ++mEdgeClash_length);
		}
	}
	for(int i = 0; i < vertex_use_length; i++) {
...
void increase_score() {
	for(int i = 1; i <= mEdgeClash_length; i++) {
...
int mTabuSearch() {
	...
	for (long long iters = mPopulationMaxVOld[mPopulation_id] + 1; iters < total_iters; iters++) {
		...
		// 当被禁忌的最小值比不被禁忌的最小值小、并且采用该move比当前最小值还小时，或者不存在不被禁忌的好move时，进入该循环；
			...
			for(int i = 0; i < s_cnt; i++) {
				...
				if(min_sub_tabu_cs < tmp_cs) {
					...
					bms_st[cs_cnt++] = tmp_tabu_st[r];
				}	
				else if(min_sub_tabu_cs == tmp_cs) bms_st[cs_cnt++] = tmp_tabu_st[r];
			}
			r = rand() % cs_cnt;
			move_id = bms_st[r].id;
			move_to = bms_st[r].c;
		...
		// 否则就选择非禁忌的操作
			...
			for(int i = 0; i < s_cnt; i++) {
				...
				if(min_sub_cs < tmp_cs) {
					cs_cnt == 0;
					min_sub_cs = tmp_cs;
					bms_st[cs_cnt++] = tmp_st[r];
				}	
				else if(min_sub_cs == tmp_cs) bms_st[cs_cnt++] = tmp_st[r];
			}
			r = rand() % cs_cnt;
			move_id = bms_st[r].id;
			move_to = bms_st[r].c;
		...
		if (total_clash <= tSminClash) {
			...
			for(int i = 1; i <= mEdgeClash_length; i++) best_clash_array[i - 1].e_id = mEdges[i];
			for(int i = 1; i <= mEdgeClash_length; i++) best_clash_array[i - 1].color = mVertexesColor[edge[mEdges[i]].x];

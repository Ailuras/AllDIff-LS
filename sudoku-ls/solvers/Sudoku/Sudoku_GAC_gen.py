...
def solve_sudoku(filename):
    ...
    return domains, simplified_count

def write_sudoku(filename, domains):
    with open(filename, 'w') as f:
        size = int(len(domains)**0.5)  # 假设数独是一个正方形，那么它的边长就是 domains 的键的数量的平方根
        f.write(f'{int(size**0.5)}\n1\n')  # 写入数独的大小
        for i in range(size):
            for j in range(size):
                if len(domains[(i, j)]) == 1:
                    f.write(str(next(iter(domains[(i, j)]))))
                else:
                    f.write('-1')
                if j != size - 1:
                    f.write('\t')
...
if __name__ == '__main__':
    filename = sys.argv[1]
    domains, simplified_count = solve_sudoku(filename)
    write_sudoku(filename, domains)
